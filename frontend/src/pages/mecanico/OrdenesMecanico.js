...
const Ordenes = () => {
    ...
    const actualizarEstadoServicio = async (servicioId, nuevoEstado) => {
        try {
            ...
            // Recargar la orden actual para obtener el estado actualizado del backend
            const response = await ordenService.getById(ordenSeleccionada.orden_id);
            ...
            showAlert('error', error.response?.data?.error || 'Error al actualizar estado del servicio');
        }
    };
                                        ...
                                        <tr key={orden.orden_id}>
                                            ...
                                            <td>
                                                <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
                                                    ...
                                                    {!['FINALIZADO', 'ENTREGADO', 'CANCELADO'].includes(orden.estado) && (
                                                        <>
                                                            <button
                                                            ...
                                                            <button
                                                                ...
                                                                onClick={() => abrirModalAgregarServicio(orden)}
                                                    ...
                                                    {orden.estado === 'FINALIZADO' && (
                                                        <button
                                                            className="btn btn-primary btn-sm"
                                                            onClick={() => actualizarEstado(orden.orden_id, 'ENTREGADO')}
                                            ...
                                            <tbody>
                                                {ordenSeleccionada.servicios.map((servicio) => (
                                                    <tr key={servicio.servicio_id}>
                                                        ...
                                                        <td>
                                                            <span className={`badge badge-${(servicio.estado || 'PENDIENTE').toLowerCase()}`}>
                                                                {servicio.estado || 'PENDIENTE'}
                                                            </span>
                                                        </td>
                                                        <td>{formatCurrency(servicio.costo || 0)}</td>
                                                        <td>
                                                            {ordenSeleccionada.estado !== 'FINALIZADO' && ordenSeleccionada.estado !== 'ENTREGADO' && (
                                                                <div style={{ display: 'flex', gap: '5px' }}>
                                                                    {(!servicio.estado || servicio.estado === 'PENDIENTE') && (
                                                                        <button
                                                                            className="btn btn-info btn-sm"
                                                                            onClick={() => actualizarEstadoServicio(servicio.servicio_id, 'EN_PROCESO')}
                                                                    ...
                                                                    {servicio.estado === 'EN_PROCESO' && (
                                                                        <button
                                                                            className="btn btn-success btn-sm"
                                                                            onClick={() => actualizarEstadoServicio(servicio.servicio_id, 'COMPLETADO')}
                                                                    ...
                                                                    {servicio.estado === 'COMPLETADO' && (
                                                                        <span style={{ color: '#10b981', fontSize: '14px' }}>✓ Completado</span>
                                                                    )}
                                                                </div>
                                                            )}
                                                        </td>
                                                ...
                                                <tr key={servicio.servicio_id}>
                                                    ...
                                                    <td>
                                                        <div style={{ display: 'flex', gap: '5px' }}>
                                                            ...
                                                            {ordenSeleccionada.servicios.length > 1 && servicio.estado !== 'COMPLETADO' && (
                                                                <button
