...
const Citas = () => {
    ...
    const confirmarAsistencia = (cita) => {
        // El mecánico siempre completa la cita (marca que el cliente asistió)
        setConfirmAction({
            cita,
            action: 'completar',
            title: '✓ Confirmar Asistencia',
            message: '¿Confirmar que el cliente asistió a la cita? Se creará una orden de reparación.',
    ...
    const ejecutarAccion = async () => {
        try {
            if (confirmAction.action === 'completar') {
                // Completar cita y crear orden
                await citaService.completar(confirmAction.cita.cita_id);
                showAlert('success', 'Cita completada y orden de reparación creada exitosamente');
                                        ...
                                        <td>
                                            <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
                                                ...
                                                {cita.estado === 'COMPLETADA' && (
                                                    <span style={{ color: '#48bb78', fontWeight: 'bold' }}>✓ Completada</span>
                                                )}
                                                {cita.estado === 'CANCELADA' && (
                                                    <span style={{ color: '#e53e3e', fontWeight: 'bold' }}>✗ Cancelada</span>
                                                )}
                                            </div>
                                        </td>
                                    </tr>
